	...
	if (username && password) {
		conn.query('SELECT * FROM registration WHERE Username = ? AND Password = ?', [username, password], function(error, results, fields) {
			...
			if (results.length > 0) {
                ...
                // const mobNumber= results[0].MobNumber;
                const token="your-token-string";
                res.cookie('token', token);
                res.cookie('username', username);
                // res.cookie('mobNumber', mobNumber);
                res.cookie('Id', Id);
...
app.put('/profile', (req, res) => {
  ...
  const sql = 'UPDATE registration SET Username = ?, MobNumber = ? WHERE id = ?';

  conn.query(sql, [username, mobileNumber, id], (err, results) => {
    if (err) {
      console.error('Error executing the query:', err);
      res.status(500).json({ error: 'Failed to update data in the profile table' });
      return;
    }
...
app.delete("/profile", (req, res) => {
  ...
  // Delete user entry from the registration table
